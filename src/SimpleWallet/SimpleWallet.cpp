...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 5334", 0 };
...
bool processServerAliasResponse(const std::string& s, std::string& address) {
	try {
		...
		// Courtesy of Investcoin Project
...
bool simple_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	...
	try {
		...
		const std::string filename = "reserve_proof_" + args[0] + "INVC.txt";
...
bool simple_wallet::export_tracking_key(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
    ...
    // This will show Tracking Key in style of Private Key Backup or Paperwallet, to prevent confusing we use above style of Investcoin like tracking keys
