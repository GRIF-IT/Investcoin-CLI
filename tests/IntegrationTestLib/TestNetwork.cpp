...
#ifdef _WIN32
const std::string investcoinDaemon = "investcoind.exe";
#else
const std::string investcoinDaemon = "investcoind";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.daemonPath = investcoinDaemon; // default
  cfg.testnet = testnet;
  cfg.logFile = "test_investcoind" + std::to_string(index) + ".log";
